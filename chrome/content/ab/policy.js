...
var abpPolicy =
{
  ...
  init: function() {
    ...
    for each (let typeName in types)
    {
      if ("TYPE_" + typeName in iface)
      {
        ...
        this.localizedDescr[this.type[typeName]] = glome.getString("abp_type_label_" + typeName.toLowerCase());
      }
    }
    
    this.type.BACKGROUND = 0xFFFE;
    this.typeDescr[0xFFFE] = "BACKGROUND";
    this.localizedDescr[0xFFFE] = glome.getString("abp_type_label_background");
    
    this.type.ELEMHIDE = 0xFFFD;
    this.typeDescr[0xFFFD] = "ELEMHIDE";
    this.localizedDescr[0xFFFD] = glome.getString("abp_type_label_elemhide");

    // whitelisted URL schemes
    this.whitelistSchemes = {};
    for each (var scheme in abpPrefs.whitelistschemes.toLowerCase().split(" "))
  ...
  refilterWindowInternal: function(wnd, start) {
    ...
    var wndData = glome.getDataForWindow(wnd);
    ...
    glome.abp.DataContainer.notifyListeners(wnd, "invalidate", data);
  },
