...
glome.abp.InvalidFilter = InvalidFilter;
...
glome.abp.CommentFilter = CommentFilter;
...
glome.abp.ActiveFilter = ActiveFilter;
...
glome.abp.RegExpFilter = RegExpFilter;
...
glome.abp.BlockingFilter = BlockingFilter;
...
glome.abp.WhitelistFilter = WhitelistFilter;
...
glome.abp.ElemHideFilter = ElemHideFilter;
...
ElemHideFilter.fromText = function(text, domain, tagName, attrRules, selector)
{
  ...
  if (!selector)
  {
    ...
    if (attrRules) {
      ...
      for each (let rule in attrRules) {
        ...
        if (separatorPos > 0) {
          ...
          if (id)
            return new InvalidFilter(text, glome.getString("abp_filter_elemhide_duplicate_id"));
          else
            id = rule;
        }
      }
    ...
    if (id)
      ...
      return new InvalidFilter(text, glome.getString("abp_filter_elemhide_nocriteria"));
  }
