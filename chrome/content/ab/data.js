...
/*
 * Stores Glome data to be attached to a window.
...
glome.abp.DataContainer = DataContainer;
...
DataContainer.prototype = {
  ...
  install: function(wnd) {
    ...
    wnd.document["glomeData" + dataSeed] = this;
  ...
  addNode: function(topWnd, node, contentType, docDomain, thirdParty, location, filter, objTab) {
    ...
    if (key in this.locations) {
      ...
      this.locations[key] = {
        ...
        location: location,
        type: contentType,
        typeDescr: abpPolicy.typeDescr[contentType],
        docDomain: docDomain,
        thirdParty: thirdParty,
        localizedDescr: abpPolicy.localizedDescr[contentType],
        filter: filter
      };
    ...
    node["glomeLocation" + dataSeed] = this.locations[key];

    if (typeof objTab != "undefined" && objTab) {
      this.locations[key].nodes.push(objTab);
      objTab["glomeLocation" + dataSeed] = this.locations[key];
    }
  ...
  removeNode: function(node) {
    if ("glomeLocation" + dataSeed in node) {
      var nodes = node["glomeLocation" + dataSeed].nodes;
...
DataContainer.getDataForWindow = function(wnd) {
  if ("glomeData" + dataSeed in wnd.document)
    return wnd.document["glomeData" + dataSeed];
  else
    return new DataContainer(wnd);
};
glome.getDataForWindow = DataContainer.getDataForWindow;

// Loads Adblock data associated with a node object
DataContainer.getDataForNode = function(node, noParent) {
  while (node) {
    if ("glomeLocation" + dataSeed in node)
      return [node, node["glomeLocation" + dataSeed]];
...
glome.getDataForNode = DataContainer.getDataForNode;
